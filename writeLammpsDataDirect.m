function [ returnValue ] = writeLammpsDataDirect(commentLine,atomNum, atomTypes, element, cellLength,charge, coordinate1, filename)
% Write lammps data file in a slow way, element is integer
% writeLammpsDataDirect(commentLine,atomNum, atomTypes, element, cellLength,
% charge, coordinate1, filename)

newFilename = filename;
fid = fopen(newFilename,'w');
if(fid == -1)
    disp('Cannot create file')
end
fprintf(fid,'%s\n',['# ' commentLine]);
fprintf(fid,'%s\n',[num2str(sum(atomNum)),' ','atoms']);
fprintf(fid,'%s\n',[num2str(atomTypes),' ','atom types']);
if(size(cellLength,2) == 1)
    fprintf(fid,'%s\n',[ num2str(cellLength([1 2])'),' xlo xhi']);
    fprintf(fid,'%s\n',[ num2str(cellLength([3 4])'),' ylo yhi']);
    fprintf(fid,'%s\n',[ num2str(cellLength([5 6])'),' zlo zhi']);
end
